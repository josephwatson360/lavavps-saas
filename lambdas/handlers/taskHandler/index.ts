...
const PLAN_TASK_DEFS: Record<string, string> = {
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  if (event.httpMethod === 'GET') {
    ...
    const taskArn   = item.task_arn as string | undefined;
    let ecsStatus   = null;
    ...
    if (taskArn && status === 'RUNNING') {
      try {
        ...
        if (task?.lastStatus === 'STOPPED' || task?.lastStatus === 'DEPROVISIONING') {
          ...
          return ok({ status: 'STOPPED', agentId, taskArn: null });
        }
      } catch {
  ...
  if (event.httpMethod === 'POST' && action === 'start') {
    const currentStatus = item.status as string ?? 'STOPPED';

    // Already running — return immediately
    if (currentStatus === 'RUNNING') {
      return ok({ status: 'RUNNING', agentId, taskArn: item.task_arn });
    }

    // Already starting — return in-progress
    if (currentStatus === 'STARTING') {
      return ok({ status: 'STARTING', agentId, message: 'Agent is already starting' });
    }

    // Check API key is configured
    if (!item.llm_secret_arn) {
      return badRequest('No LLM API key configured. Add an API key before starting the agent.');
    }

    // Determine task definition for plan
    const taskDefFamily = PLAN_TASK_DEFS[planCode] ?? PLAN_TASK_DEFS.starter;
    const iamTaskRole   = item.iam_task_role_arn as string | undefined;

    if (!iamTaskRole) {
      return internalError('Agent task role not provisioned. Contact support.');
    }
    ...
    // Launch ECS task
    const efsApId = item.efs_access_point_id as string;
    ...
    try {
      ...
      return internalError('Failed to start agent. Please try again.');
    }
  }
  ...
  if (event.httpMethod === 'POST' && action === 'stop') {
    ...
    return ok({ status: 'STOPPED', agentId });
  }
