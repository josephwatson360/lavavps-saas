...
// ─────────────────────────────────────────────────────────────────────────────
const logger = (0, logger_1.createLogger)('taskWatcherLambda');
...
const handler = async () => {
    ...
    const runningResult = await dynamo.send(new lib_dynamodb_1.QueryCommand({
    ...
    for (const task of runningTasks) {
        ...
        const planCode = task.plan_code ?? 'starter';
        const taskArn = task.task_arn;
        // ── Check idle timeout ──────────────────────────────────────────────
        const idleTimeoutMin = task.idle_timeout_min
