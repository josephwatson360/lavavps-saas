...
const logger = createLogger('configRenderer');
...
export const handler = async (event: { tenantId: string; agentId: string }): Promise<void> => {
  ...
  const configVersionAttempted = (item.config_version ?? 0) as number;
  const priorValidVersion       = (item.last_valid_config_version ?? 0) as number;
  ...
  if (item.config_primaryModel) {
    config.agents.defaults.model = {
      primary: item.config_primaryModel as string,
    };
  }

  // System prompt
  if (item.config_systemPrompt) {
    config.agents.defaults.systemPrompt = item.config_systemPrompt as string;
  }

  // Temperature
  if (item.config_temperature !== undefined) {
    config.agents.defaults.temperature = item.config_temperature as number;
  }

  // Max tokens (floor 256 enforced by configHandler; no plan ceiling)
  if (item.config_maxTokens !== undefined) {
    config.agents.defaults.maxTokens = item.config_maxTokens as number;
  }

  // Agent name
  if (item.config_agentName) {
    config.agents.defaults.agentName = item.config_agentName as string;
  }
  ...
  if (item.config_sessionResetMode) {
    config.agents.defaults.session = {
      reset: {
        mode: item.config_sessionResetMode as string,
        ...(item.config_sessionResetMode === 'idle' && item.config_sessionIdleMinutes
          ? { idleMinutes: item.config_sessionIdleMinutes as number }
          : {}),
      },
    };
  }
  ...
  if (item.config_discordBotToken) {
    channels.discord = {
      enabled:  true,
      botToken: item.config_discordBotToken as string,
      ...(item.config_discordGuildId ? { guildId: item.config_discordGuildId as string } : {}),
    };
    hasChannels = true;
  }
  ...
  if (item.config_telegramBotToken) {
    channels.telegram = {
      enabled:  true,
      botToken: item.config_telegramBotToken as string,
    };
    hasChannels = true;
  }
  ...
  if (item.config_whatsappPhoneNumberId && item.config_whatsappAccessToken) {
    channels.whatsapp = {
      enabled:        true,
      phoneNumberId:  item.config_whatsappPhoneNumberId as string,
      accessToken:    item.config_whatsappAccessToken as string,
    };
    hasChannels = true;
  }
...
// ── Config Validation ─────────────────────────────────────────────────────────
// Validates the structural integrity of the rendered config.
// This is NOT a full OpenClaw schema validation (which runs on the container)
// but catches the most common mistakes before they reach EFS.

type ValidationError = { field: string; error: string; allowed?: unknown };

function validateOpenClawConfig(config: OpenClawConfig): ValidationError[] {
