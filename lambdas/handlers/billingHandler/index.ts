...
async function getPriceIds(): Promise<Record<string, string>> {
  try {
    const res = await sm.send(new GetSecretValueCommand({ SecretId: PRICE_IDS_SECRET }));
...
export async function handler(
  ...
  if (method === 'POST' && path.endsWith('/billing/checkout')) {
    ...
    if (type === 'plan') {
      if (!planCode) return err('planCode is required for plan checkout');
      ...
      if (!storageGb) return err('storageGb is required for storage add-on');
